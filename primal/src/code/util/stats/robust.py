...
"""
TAKEN FROM LSL: http://fornax.phys.unm.edu/lwa/trac/
SEE COPYRIGHT THEREIN.
...
def std(inputData, Zero=False, debug=False):
	"""
	...
	good = good[0]
	if len(good) < 3:
		if debug: print "WARNING:  Distribution is too strange to compute standard deviation"
		sigma = -1.0
		return sigma
