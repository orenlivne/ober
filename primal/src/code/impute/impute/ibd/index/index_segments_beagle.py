...
numpy as np, time, operator, pickle
...
# Methods
#---------------------------------------------
def ibd_edge_weight(x, a, b, m0=1.5 * MEGA_BASE_PAIR, m_inf=0.4 * MEGA_BASE_PAIR, L_half_life=2.0 * MEGA_BASE_PAIR, mu=0.001):
...
def __parse_command_line_args():
    ...
    parser.add_option('-r', '--region-size', type='int', dest='region_size', default=100,
    ...
    parser.add_option('-l', '--min-length', type='float', dest='min_len', default=0.4,
    ...
    parser.add_option('-a', '--algorithm', type='str', dest='algorithm', default='amg',
                      help='Quasi-clique partitioning algorithm: naive|amg.')
    parser.add_option('-t', '--threshold-affinity', type=float, dest='theta_affinity', default=0.95,
                      help='Quasi-clique partitioning edge dropping affinity threshold.')
    parser.add_option('-u', '--threshold-weight', type=float, dest='theta_weight', default=0.5,
                      help='Quasi-clique partitioning edge dropping graph weight threshold.')
...
def _save_index(region_info, result, out_dir, options):
    ...
    for r in region_info:
        ...
        # IBD-group-to-haplotype index
        groups = np.array([[(np.array(group, dtype=np.int) if group is not None else None) for group in groups] 
                           for groups in (x[1] for x in block)])
...
def _estimated_call_rate(cliques, R, num_samples):
...
def _not_covered(cliques, R, num_samples):
    '''Return the list of cliques and list of haplotypes not covered by imputation from the haplotype set R.'''
    imputable_haps = sum((c for c in cliques if set(c) & R), [])
    # Samples that cannot be fully imputed from R
    return np.array([sample for sample in xrange(num_samples)
                     if not ((sample, PATERNAL) in imputable_haps and (sample, MATERNAL) in imputable_haps)]), \
    np.array([k for k, c in enumerate(cliques) if not set(c) & R])
    ...
    def __init__(self, info, segment_file, regions, options):
        ...
        r = np.zeros((num_regions,),
                     dtype=[('region', np.uint8),  # Region index
                            ('bp_start', np.int),  # region start base-pair position
                            ('bp_stop', np.int),  # region end base-pair position
                            ('snp_start_ext', np.int),  # extended region (with margins) start base-pair position
                            ('snp_stop_ext', np.int),  # extended region (with margins) stop base-pair position
                            ('snp_start', np.int),  # region start base-pair SNP index
                            ('snp_stop', np.int),  # region end base-pair SNP index
                            ('num_snps', np.int)  # region end SNP index
                            ])
        ...
        for region_info in self.region_info:
            region, start, stop = region_info['region'], region_info['snp_start'], region_info['snp_stop']
...
class _SnpProcessor(object):
    ...
    def process(self, (region, snp)):
        ...
        if options.debug >= 1:
            ...
            call_rate_hap, call_rate_genotype = _estimated_call_rate(cliques, self.R, info.num_samples) 
    ...
    def _new_partitioner(self, options):
        ...
        elif algorithm == 'amg': return lambda G: partition_amg.partition(G, theta=options.theta_affinity, theta_weight=options.theta_weight)
...
def __main(args, options):
    ...
    try:
        ...
        _writeln('regions ' + repr(regions) + ' num_regions ' + repr(num_regions) + 
                 ' segment length threshold ' + repr(options.min_len) + ' Mbp algorithm ' + options.algorithm + 
                 ' margin ' + repr(options.margin) + ' affinity threshold ' + repr(options.theta_affinity) + 
                 ' weight threshold ' + repr(options.theta_weight), lock)
        ...
        if options.num_processes > 1:
            ...
            result = po.imap(snp_processor.process, ((region, snp) for region, snp in snps))
...
def main(in_file, info_file, segment_file, out_dir, **kwargs):
    '''Main program - reads options from a method parameter dictionary.'''
    # Default options
    options = util.Struct(num_processes=1, region_size=100, regions=None, snp_index=None, profile=False,
                          min_degree=4, min_len=0.4, margin=0.8, debug=0, algorithm='amg',
                          theta_affinity=0.95, theta_weight=0.5)
    # Override with passed arguments
    options.update(**kwargs)
    
    args = in_file, info_file, segment_file, out_dir
