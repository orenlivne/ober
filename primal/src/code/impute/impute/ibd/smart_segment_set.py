...
class PairSegment(interval_tree.Interval):
    ...
    '''Object equality.'''
    def __key(self): return (self.snp_start, self.snp_stop, self.sample0, self.sample1)
...
class SmartSegmentSet(object):
    ...
    '''Object equality.'''
    def __key(self): return [x.intervals for x in self._index]
    def __hash__(self): return hash(self.__key())
    def __eq__(self, other): return self.__key() == other.__key()
    def __ne__(self, other): return self.__key() != other.__key()
    ...
    '''Methods to allow un/pickling.'''
    def __getstate__(self):
        return {'_n': self._n, '_index': self._index }
    ...
    def save(self, out, samples=None):
        ...
        s = np.array([(x.snp_start, x.snp_stop, x.bp_start, x.bp_stop) + x.sample0 + x.sample1 
                      for lst in self._index for x in lst.intervals if x.sample0 < x.sample1])
    ...
    def find(self, start, stop, source, target=None):
        ...
        return [s for s in intersecting if s.sample0 == source and s.sample1 in target] if target is not None \
                else intersecting
...
####################################################################################
class PlainSegmentSet(object):
    '''Defines a segment set without search capabilities, indexed by pairs of individuals.'''
    __slots__ = ('_n', '_index')
    ...
    def __init__(self, n, segments):
        ...
        # Accumulate segments into lists
        self._n = n
        index = [[SegmentSet() for _ in xrange(n)] for _ in xrange(n)]
        for x in segments:
            index[x.sample0][x.sample1].append(x)
            index[x.sample0][x.sample1].append(x)
        self._index = index
    ...
    '''Methods to allow un/pickling.'''
    def __getstate__(self): return {'_n': self._n, '_index': self._index }
    def __setstate__(self, state):
        for key, value in state.iteritems(): setattr(self, key, value)

    @property
    def size(self): return sum(y.size for x in self._index for y in x) / 2
    ...
    def from_list(n, s):
        ...
        Each segment is assumed to be between a PAIR of haplotypes only.
        Only list-type ''samples'' fields are supported.'''
        return PlainSegmentSet(n, [PairSegment((x[4], x[5]), (x[6], x[7]), x[0], x[1], x[2], x[3]) for x in s]) 
    ...
    def load(n, inp):
        ...
        Each segment is assumed to be between a PAIR of haplotypes only.
        Only list-type ''samples'' fields are supported.'''
        return PlainSegmentSet.from_list(n, np.loadtxt(inp, dtype=np.uint))
