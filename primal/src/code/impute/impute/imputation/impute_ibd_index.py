...
using an IBD segment dictionary (a SegmentIndex). Hopefully
much faster than impute_ibd while giving similar or better
call rates and the same concordance.

Created on March 20, 2013
@author: Oren Livne <livne@uchicago.edu>
============================================================
'''
import impute as im, numpy as np, time, itertools
...
# Methods
#---------------------------------------------
def impute(ibd, t, snp=None, majority_threshold=0.66, debug=1, samples=None, genotype=None,
           debug_sample= -1, ibd_sample_index=None, remove_partial_calls=False):
    ...
    for snp_index in snp:
        ...
        t_start = time.time()
        _IbdIndexImputer(imputed_data[snp_index], imputed_hap_type[snp_index],
                         ibd, t.training_data[snp_index], t.sample_index,
                         chrom, snp_bp, debug=(debug >= 2), majority_threshold=majority_threshold,
                         debug_sample=debug_sample, remove_partial_calls=remove_partial_calls).impute(samples)           
...
class _IbdIndexImputer(object):
    ...
    def __init__(self, h, hap_type, ibd, g, training_sample_index, chrom, snp_bp, debug=False, majority_threshold=0.66,
                 debug_sample= -1, max_iter=1000, remove_partial_calls=False):
        ...
        self.debug_sample, self.chrom, self.remove_partial_calls = h, hap_type, g, ibd, training_sample_index, max_iter, debug, \
        debug_sample, chrom, remove_partial_calls
